...
\section{Proposal}
    ...
    % The compilation stages mainly consists of lexing, parsing, and instruction optimization which is NP-hard.
    % Mathematically, the optimization is NP-hard.
    % The search space for optimizations, while large, is typically constrained to the instruction set and a relatively small number of optimization passes like register allocation and instruction scheduling.
    ...
    The compiler at this stage must optimize the ordering of the machine instructions to take advantage of pipelining, register allocation.
    ...
    Furthermore, compiler makers have developed highly efficient heuristics to approximate the best machine code within a reasonable time.

    On the other hand, the search space for the EDA stages are much more massive. 
    These are: synthesis, placement, and routing, and all three are NP-hard with massive search spaces.
