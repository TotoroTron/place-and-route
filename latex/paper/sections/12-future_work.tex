...
Figure \ref{fig:net_model} shows an example of hyperedge reduction using the star and clique models.
...
The weight \(w_{i,j}\) is the weight of the 2-pin edge connecting nodes $i$ and $j$. 
Its value depends on both the net model and the objective:

\begin{itemize}
    \item \textbf{Star/Clique models with a quadratic (squared-distance) objective:}  
    Use constant edge weights. In the star model, each reduced 2-pin edge is given unit weight $w_{ij}=1$.  
...
We can cast the squared formulation into quadratic form. 
In the x-direction this can be written conveniently as
\begin{equation}
