...
In force directed moves, there is a concept of global placement and detailed placement, where global placement is driven by forces and detailed placement attempts to legalize the global placement by discretizing it onto the grid of \texttt{Site}s on the device.
In such placers, objects are first placed freely on an analog field in global placement, even allowing objects to overlap with one another. 
Then, in detailed placement, the object positions are discretized amongst the grid of resources and overlapping objects spread out until there are no overlaps.
In literature, detailed placement is also referred to as legalization.
