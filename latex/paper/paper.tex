...
\section{RapidWright API}
    \begin{figure}
        \centering
        \includegraphics[width=6.0cm]{figures/vivado_dcps.png}
        \caption{Design flow showing how RapidWright can take a design checkpoint, allow user customization, and reintegrate it back into the design flow.}
    ...
    There are three packages of objects that are most relavent to building our placer: the \textbf{edif} package, \textbf{design} package, and \textbf{device} package. 
    Figure \ref{fig:routing_objects} shows the device routing objects as shown in the Vivado device viewer while figure \ref{fig:edif_netlist} shows the EDIF netlist objects as shown in the Vivado netlist viewer.
    ...
    \begin{figure*}[!htt]
        \centering
        \includegraphics[width=12.0cm]{figures/edif_netlist.png}
        \caption{Examples of edif netlist objects for a Xilinx FPGA}
        \label{fig:edif_netlist}
    \end{figure*}
...
\section{Placement Implementation}
    ...
    With a basic understanding of FPGA architecture, design placement, and RapidWright, we can begin implementing our SA placer. 
    ...
    \subsection{PrePacking}
        ...
        We specifically look for CARRY chains, DSP cascades, and LUT-FF pairs as these patterns are most prevalent in all FPGA designs.

        \subsection{CARRY Chains}
            To find CARRY chains, we take the CARRY4 group and follow the below pseudocode. 
            \begin{verbatim}
                for (EDIFHierCellInst ehci : CARRYCells) {
                    ehci.getPorts()
