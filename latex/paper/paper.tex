...
\section{A Brief History on FPGA Architecture}
    ...
    First we will briefly outline the evolution of configurable logic architecture as well as the FPGA design flow. 
...
\section{Xilinx 7-Series Architecture}
    ...
    In this paper we will be working with the Xilinx 7-Series architecture which follows a heterogenous island-style design. 
    ...
    Note that in the Xilinx architectures, "CLBs" refer to CLB Tiles, each containing two "SLICE" Sites, with each SLICE Site containing a collection of BELs - 8 LUTs, 8 Flip Flops, and 1 CARRY4. 
    ...
    In the SLICEL, Lookup Tables (LUTs), as their name implies, are fast shallow read-only memories (ROMs) with 6 input pins and 1 or 2 output pins. 
    ...
    The SLICEM is similar to the SLICEL, the key difference being that their LUTs can optionally be programmed as RAMs to facilitate "Distributed RAM", an alternative to the larger dedicated Block RAM (BRAM) macro. 
    Xilinx devices usually come a 25\% SLICEM, 75\% SLICEL split amongst all SLICE Sites, where a CLB Tile can contain either two SLICELs or one SLICEL one SLICEM. 
...
\section{The FPGA Design Flow and Toolchain}
    ...
    In addition to the FPGA hardware architecture, one should also understand the role that placement plays in the borader FPGA design flow and software toolchain. 
