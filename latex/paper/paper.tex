...
\section{Introduction}
    ...
    However, readers who are already familiar with these concepts can skip directly to the RapidWright API section. 
...
\section{A Brief History on FPGA Architecture}
    ...
    First we will briefly review the evolution of configurable logic architecture as well as general FPGA design flow. 
...
\section{Xilinx 7-Series Architecture}
    ...
    \begin{figure*}[]
        \centering
        \includegraphics[width=15.0cm]{figures/hierarchy_3.png}
        \caption{Architecture Hierarchy of a Xilinx FPGA}
        \label{fig:hierarchy_4}
    \end{figure*}

    In this paper we will be working with the Xilinx 7-Series devices which follows a heterogenous island-style architecture. 
    The 7-Series architecture first released in 2010 and was then superceded in 2013 by the Ultrascale architecture which offered several improvements over the 7-Series. 
    Although the latest FPGAs follow the Ultrascale architecture, this paper focuses on the 7-Series FPGAs due to their greater accessibility to hobbyists and compatibility with open source tooling. 

    Figure \ref{fig:hierarchy_4} illustrates the architectural hierarchy of a 7-Series Xilinx FPGA. 
    At its most basic level, an FPGA is a vast physical array of atomic components known as Basic Elements of Logic (BELs). 
    ...
    In the 7-Series architecture, "CLBs" refer to CLB Tiles, each containing two "SLICE" Sites, with each SLICE Site containing a collection of BELs - 8 LUTs, 8 Flip Flops, and 1 CARRY4. 


    The LUTs and Flip Flops constitute the core of the FPGA programmable logic. 

    Flip Flops (FFs) facilitate synchronous (clock-driven or event-driven) registers. 

    SLICE Sites come in two flavors - SLICEL (SLICE-Logic) and SLICEM (SLICE-Memory). 
    In the SLICEL, Lookup Tables (LUTs) are fast shallow read-only memories (ROMs) with 6 input pins and 1 or 2 output pins. 
    All combinational logic - boolean functions - are mapped onto these LUTs as read-only-memories during synthesis. 
    Larger boolean functions with more than 6 inputs will be mapped onto multiple cascaded LUTs. 

    The SLICEM is similar to the SLICEL, with the key difference that their LUTs can optionally be programmed as RAMs to facilitate "Distributed RAM", an alternative to the larger dedicated Block RAM (BRAM) macro. 

    Shown in Figure \ref{fig:slicel} and Figure \ref{fig:slicem} are a SLICEL and SLICEM respectively as seen in the Vivado device viewer. 

    Xilinx devices usually come a 25\% SLICEM, 75\% SLICEL split amongst all SLICE Sites, where a CLB Tile can contain either two SLICELs or one SLICEL plus one SLICEM. 

    Each SLICE also comes with a shallow 4-bit carry-lookahead adder (CLA) BEL called "CARRY4". 

\section{The FPGA Design Flow and Toolchain}
    \begin{figure}
        \centering
        \includegraphics[width=8.0cm]{figures/design_flow.png}
        \caption{A typical FPGA design and verification workflow}
        \label{fig:design_flow}
    \end{figure}

    In addition ot the FPGA architecture, one should also understand the role that placement plays in the broader FPGA design flow and software toolchain. 

    Figure \ref{fig:design_flow} illustrates a typical FPGA design and verification workflow, transforming a high-level digital design into a configuration file that programs the physical FPGA.

    The process begins with \textbf{design entry}, where an engineer describes the desired functionality of a digital system using a hardware description language (HDL) such as Verilog or VHDL. 
    Engineers may use various coding styles - behavioral, structural, dataflow, or a mix of all three - to capture the logic and behavior of the design while abstracting away the intricacies of the physical hardware architecture. 

    Next, the user runs the \textbf{synthesis} tool, which parses the raw HDL source files to infer logical behavior, perform optimizations, and instantiate primitive cells. 
    The result is a detailed netlist, typically output as a low-level Verilog or Electronic Design Interchange Format (EDIF) file, where the design is represented purely in structural HDL. 
    The final stage is often referred to as "technology mapping", as the vocabulary or library of primitive cells must be specific to the FPGA architecture and consequently the FPGA manufacturer. 
    ...
    The objective is to place the logical cell instances of the netlist onto physical BELs on the device in a way that minimizes wirelength, simplifies routing, and satisfies timing constraints (signal propagation delay). 
    ...
    Placement and routing are generally referred jointly as the \textbf{implementation} stage, as both stages are interdependent and must adhere to the constraints of the same device. 
    State of the art (SOTA) tools will perform routing-aware placement or placement-fluid routing for greater optimization. 

    The final stage is the \textbf{bitstream} generation, where a vendor-specific programming tool takes the implemented design and generates a configuration file or bitstream that programs the physical FPGA device. 
...
\section{Placement Implementation via RapidWright API}

    %%%


    The RapidWright API offers an additional workflow column in parallel to both the design flow and verification flow. 
